...
Album Album::fromBson(const bsoncxx::document::view& doc) {
    ...
    try {
        if (doc["_id"] && doc["_id"].type() == bsoncxx::type::k_oid) {
            album.id = doc["_id"].get_oid().value.to_string();
        }
        if (doc["title"]) album.title = std::string(doc["title"].get_utf8().value);
        if (doc["description"]) album.description = std::string(doc["description"].get_utf8().value);
        if (doc["admin_token"]) album.admin_token = std::string(doc["admin_token"].get_utf8().value);
        if (doc["status"]) album.status = albumStatusFromString(std::string(doc["status"].get_utf8().value));
        if (doc["image_count"]) album.image_count = doc["image_count"].get_int32().value;
        if (doc["public_image_count"]) album.public_image_count = doc["public_image_count"].get_int32().value;
        if (doc["uploader_name"]) album.uploader_name = std::string(doc["uploader_name"].get_utf8().value);
        if (doc["created_by"]) album.created_by = std::string(doc["created_by"].get_utf8().value);
        if (doc["created_at"]) album.created_at = std::string(doc["created_at"].get_utf8().value);
        if (doc["updated_at"]) album.updated_at = std::string(doc["updated_at"].get_utf8().value);
        if (doc["published_at"]) album.published_at = std::string(doc["published_at"].get_utf8().value);
        if (doc["token_used"]) album.token_used = doc["token_used"].get_bool().value;
        ...
        if (doc["images"] && doc["images"].type() == bsoncxx::type::k_array) {
            for (const auto& elem : doc["images"].get_array().value) {
                if (elem.type() == bsoncxx::type::k_document) {
                    album.images.push_back(Image::fromBson(elem.get_document().view()));
                }
            }
        }
...
bsoncxx::document::value Album::toBson() const {
    auto imageArray = array{};
    for (const auto& img : images) {
        imageArray << img.toBson().view();
    }
