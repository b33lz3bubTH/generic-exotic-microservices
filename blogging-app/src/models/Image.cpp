...
Image Image::fromBson(const bsoncxx::document::view& doc) {
    ...
    try {
        if (doc["_id"]) {
            image.id = bsoncxx::to_json(doc["_id"].get_document());
        }
        if (doc["album_id"]) {
            image.album_id = std::string(doc["album_id"].get_utf8().value);
        }
        if (doc["url"]) {
            image.url = std::string(doc["url"].get_utf8().value);
        }
        if (doc["alt_text"]) {
            image.alt_text = std::string(doc["alt_text"].get_utf8().value);
        }
        if (doc["caption"]) {
            image.caption = std::string(doc["caption"].get_utf8().value);
        }
        if (doc["display_order"]) {
            image.display_order = doc["display_order"].get_int32().value;
        }
        if (doc["status"]) {
            image.status = imageStatusFromString(std::string(doc["status"].get_utf8().value));
        }
        if (doc["nsfw_flagged"]) {
            image.nsfw_flagged = doc["nsfw_flagged"].get_bool().value;
        }
        if (doc["nsfw_reason"]) {
            image.nsfw_reason = std::string(doc["nsfw_reason"].get_utf8().value);
        }
        if (doc["created_at"]) {
            image.created_at = std::string(doc["created_at"].get_utf8().value);
        }
        if (doc["updated_at"]) {
            image.updated_at = std::string(doc["updated_at"].get_utf8().value);
        }
